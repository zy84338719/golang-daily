...
func getCookieStr(username_text string, password_text string) string {
	...
	if nil != err {
		log.Error("start a chromedriver service falid", err.Error())
	...
	if err != nil {
		log.Error("connect to the webDriver faild", err.Error())
	...
	if err != nil {
		log.Error("get page faild", err.Error())
	...
	if err != nil {
		log.Error("get username faild", err.Error())
...
func Login() error{
	...
	if err != nil {
		log.Error("account.txt is not found")
		return err
	...
	for scanner.Scan() {
		...
		log.Info("start login username:", text[0])
